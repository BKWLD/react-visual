...
describe("no asset", () => {
  it("renders nothing", () => {
    cy.mount(
      <ReactVisual width={300} height={200} alt="" data-cy="react-visual" />,
    );
...
describe("fixed size", () => {
  ...
  it("renders video", () => {
    cy.mount(
      <ReactVisual
        video="https://github.com/BKWLD/react-visual/raw/refs/heads/main/packages/react/cypress/fixtures/600x400.mp4"
        width={300}
        height={200}
        alt=""
      />,
...
describe("natural size", () => {
  it("renders image", () => {
    cy.mount(<ReactVisual image="https://placehold.co/200x200.png" alt="" />);
...
describe("srcset", () => {
  ...
  it("doesn't use imageSizes when sizes == 100vw", () => {
    ...
    cy.get("[srcset]").invoke("attr", "srcset").should("not.contain", " 16w");
  });
  ...
  it("supports rendering object based sources", () => {
    ...
    cy.mount(
      <ReactVisual
        ...
        video={{
          landscape: {
            url: "https://github.com/BKWLD/react-visual/raw/refs/heads/main/packages/react/cypress/fixtures/500x250.mp4#landscape",
            aspect: 2,
          },
          portrait: {
            url: "https://github.com/BKWLD/react-visual/raw/refs/heads/main/packages/react/cypress/fixtures/500x500.mp4#portrait",
            aspect: 1,
          },
        }}
        sourceTypes={["image/webp"]}
        ...
        imageLoader={({ src, type, media, width }) => {
          ...
          const text =
            new URL(asset.url).searchParams.get("text") +
            `\\n${dimensions}${ext}`;

          // Make the url
          return (
            `https://placehold.co/${dimensions}${ext}?text=` +
